...
public class ThreeHeap implements PriorityQueue {
	...
	public void insert(double x) {
		data[size] = x;
		int index = size;
		while (data[index] < data[(index-1)/3]) {
			swap(index, (index-1)/3);
			index = (index-1)/3;
		}
		size++;
		if (size == data.length) {
	...
	public boolean checkValid() {
		for (int i = 0; i < size; i++) {
			if ((i+1)*3-2 < size && data[i] > data[(i+1)*3-2] || (i+1)*3-1 < size && data[i] > data[(i+1)*3-1] || (i+1)*3 < size && data[i] > data[(i+1)*3]) {
				return false;
			}
		}
		return true;
	 ...
	 * 
	 * @return the index of the smallest child (returns (i+1)*3-1 if there is no child)
	 */
	private int minChildIndex(int i) {
		if ((i+1)*3-2 >= data.length || (i+1)*3-1 >= data.length) {
			return (i+1)*3-2;
		}
		if ((i+1)*3 >= data.length) {
			Double one = data[(i+1)*3-2];
			Double two = data[(i+1)*3-1];
			if (two == null || one == null) {
				return (i+1)*3-2;
			} else {
				return one > two ? (i+1)*3-1 : (i+1)*3-2;
			}
		}
		Double one = data[(i+1)*3-2];
		Double two = data[(i+1)*3-1];
		Double three = data[(i+1)*3];
		if (two == null || one == null) {
			return (i+1)*3-2;
		} else if (three == null) {
			return (i+1)*3-1;
			...
			if (one < two) {
				return one < three ? (i+1)*3-2 : (i+1)*3;
			} else {
				return two < three ? (i+1)*3-1 : (i+1)*3;
			}
		}
	}
}
